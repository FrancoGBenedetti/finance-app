...
// Botón secundario reutilizable (Reiniciar / Descargar)
function SecondaryBtn({ onClick, children }) {
  return (
    <button
      onClick={onClick}
      className="flex-1 sm:flex-none shrink-0 bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white border border-gray-700 text-sm font-medium px-3 py-1.5 rounded-lg transition-colors"
    >
      {children}
    </button>
  )
...
export default function Header() {
  ...
  function handleDownload() {
    const { incomes, expenses, credits, savings, transactions } = useFinanceStore.getState()
    downloadCSV({ incomes, expenses, credits, savings, transactions })
  }
  ...
  return (
    <>
      <header className="sticky top-0 z-20 bg-gray-950/95 backdrop-blur border-b border-gray-800/80">
        ...
        {/* Fila 4 — Controles principales */}
        <div className="px-4 sm:px-6 py-2 border-b border-gray-800/40">
          {/* Fila superior: Toggle + [Reiniciar+Descargar en desktop] + Crear */}
          <div className="flex items-center gap-2">
            ...
            {/* Reiniciar + Descargar — solo visibles en desktop en esta fila */}
            <SecondaryBtn onClick={() => setResetOpen(true)}>
