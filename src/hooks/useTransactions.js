...
function applySourceUpdate(batch, sideEffect) {
  ...
  } else if (sideEffect.type === 'debit') {
    batch.update(doc(db, 'debits', sideEffect.id), { amount: sideEffect.newAmount })
  } else {
...
export function useTransactions() {
  ...
  // ── Debits ─────────────────────────────────────────────────────────────────
  async function addDebito(data) {
    const error = validateDebito(data)
    if (error) throw new Error(error)
    const ref = doc(collection(db, 'debits'))
    const batch = writeBatch(db)
    batch.set(ref, {
      title: data.title,
      amount: Number(data.amount),
  ...
  async function updateDebito(id, data) {
    const error = validateDebito(data)
    if (error) throw new Error(error)
    const batch = writeBatch(db)
    batch.update(doc(db, 'debits', id), { title: data.title, amount: Number(data.amount) })
    await batch.commit()
  }

  async function removeDebito(id) {
    const { transactions } = useFinanceStore.getState()
    if (transactions.some((t) => t.sourceId === id && t.sourceType === 'debit'))
      throw new Error('No se puede eliminar un débito que tiene transacciones vinculadas')
    const batch = writeBatch(db)
    batch.delete(doc(db, 'debits', id))
    await batch.commit()
  }
  ...
  async function removeExpense(id) {
    const { incomes, debits, credits, transactions } = useFinanceStore.getState()
    const expenseTransactions = transactions.filter((t) => t.expenseId === id)
    const reversals = computeExpenseDeleteReversals(expenseTransactions, incomes, credits, debits)
    const batch = writeBatch(db)
  ...
  async function addTransaction(data) {
    ...
    const error      = validateTransaction(payload, incomes, credits, debits)
    if (error) throw new Error(error)
    const sideEffect = computeTransactionSideEffects(payload, incomes, credits, debits)
  ...
  async function removeTransaction(id) {
    ...
    const reversal = computeTransactionReversal(transaction, incomes, credits, debits)
    const batch    = writeBatch(db)
  ...
  async function resetEntities(entitiesToReset) {
    ...
    for (const { id, type } of entitiesToReset) {
      if (type === 'expense') {
        ...
        if (inc?.initialAmount !== undefined)
          batch.update(doc(db, 'incomes', id), { amount: inc.initialAmount })
      } else if (type === 'debito') {
        const dbt = debits.find((d) => d.id === id)
        if (dbt?.initialAmount !== undefined)
          batch.update(doc(db, 'debits', id), { amount: dbt.initialAmount })
      } else if (type === 'credit') {
        const crd = credits.find((c) => c.id === id)
        if (crd?.limit !== undefined)
  ...
  return {
    ...
    addDebito,    updateDebito,    removeDebito,
